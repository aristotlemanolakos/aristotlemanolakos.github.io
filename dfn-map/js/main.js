...
;(function(window) {
	...
	var support = {transitions : Modernizr.csstransitions},
		...
		// pins
		pins = [].slice.call(mallLevelsEl.querySelectorAll('a.pin')),
		// content element
	...
	function showSpace(sliding) {
		...
		// remove last decimal from ref, if it is in three parts, 
		var pinRef = spaceref.split(".", 2).join(".");
		// map pin gets selected
		classie.add(mallLevelsEl.querySelector('.pin[data-space="' + pinRef + '"]'), 'pin--active');
	}
	...
	function hideSpace() {
		...
		classie.remove(mallLevelsEl.querySelector('.pin[data-space="' + pinRef + '"]'), 'pin--active');
		// remove class active (if any) from current list item
		var activeItem = spacesEl.querySelector('li.list__item--active');
		if( activeItem ) {
			classie.remove(activeItem, 'list__item--active');
